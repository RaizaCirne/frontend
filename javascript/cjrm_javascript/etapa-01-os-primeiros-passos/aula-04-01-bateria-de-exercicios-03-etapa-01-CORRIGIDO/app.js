...
brazilianFoods = brazilianFoods.concat("Coxinha", null, "Brigadeiro");
...
- Substitua "NUMERO_DE_ITENS" pelo valor correto, mas sem inserir o  número diretamente;
- Substitua as palavras "NOME_DO_ITEM_X" pelas informações corretas, mas sem digitar os nomes das comidas diretamente;
...
forma que caso a quantidade de itens do array aumente ou diminua, o último item continue sendo exibido;
- Para testar essa lógica, na linha acima do console.log que  
você acabou de escrever, remova o último item do array "dogNames"  utilizando o método adequado para isso;
...
- Adicione o item "Nina" no final do array "dogNames", utilizando  o método adequado para isso;
- Agora, armazene em uma const "initials" uma template string com  apenas as letras iniciais dos nomes do array "dogNames";
...
const initials = `${dogNames[0].slice(0, 1)}${dogNames[1].slice(
  0,
  1
)}${dogNames[2].slice(0, 1)}${dogNames[3].slice(0, 1)}`;
...
- Exiba no console a string abaixo, substituindo "SOBREMESA" pela constante que você criou.
